...
import { classQueries, teacherQueries, User as TeacherUser } from '@/lib/database-queries';
  ...
  const handleAddHomework = async () => {
    ...
    try {
      await addDoc(collection(db, 'homeworks'), {
        ...
        teacherId: newHomework.teacherId || user.uid,
        teacherName: newHomework.teacherName || user.email || 'Admin',
  ...
  const handleEditHomework = async () => {
    ...
    try {
      await updateDoc(doc(db, 'homeworks', editingHomework.id), {
        ...
        subject: editingHomework.subject,
        class: editingHomework.class,
        teacherId: editingHomework.teacherId,
        teacherName: editingHomework.teacherName,
        dueDate: editingHomework.dueDate,
  ...
  if (loading) {
    return (
      <ProtectedRoute allowedRoles={['admin', 'super_admin']}>
        <AdminLayout title="বাড়ির কাজ ব্যবস্থাপনা" subtitle="শিক্ষকদের প্রকাশিত বাড়ির কাজ নিয়ন্ত্রণ করুন">
            ...
            <div className="bg-white rounded-xl shadow-2xl max-w-3xl w-full max-h-[90vh] overflow-hidden flex flex-col animate-modal-enter">
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-6">
                ...
                <div>
                  ...
                  <select
                    value={newHomework.teacherId}
                    onChange={(e) => {
                      const selectedTeacher = teachers.find(t => t.uid === e.target.value);
                      setNewHomework({ 
                        ...newHomework, 
                        teacherId: e.target.value,
                        teacherName: selectedTeacher ? (selectedTeacher.name || selectedTeacher.displayName || '') : ''
                      });
                    }}
                    className="w-full px-4 py-3 border-2 border-gray-300 rounded-lg focus:outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-200 transition-colors bg-white"
                    required
                  >
                    ...
                    {teachers.map((teacher) => (
                      <option key={teacher.uid} value={teacher.uid}>
                        {teacher.name || teacher.displayName || teacher.email || 'Unknown Teacher'}
                      </option>
                    ))}
                  </select>
                </div>
            ...
            <div className="bg-white rounded-xl shadow-2xl max-w-3xl w-full max-h-[90vh] overflow-hidden flex flex-col animate-modal-enter">
              ...
              <div className="flex-1 overflow-y-auto p-6 space-y-6">
                ...
                <div>
                  ...
                  <select
                    value={editingHomework.teacherId}
                    onChange={(e) => {
                      const selectedTeacher = teachers.find(t => t.uid === e.target.value);
                      setEditingHomework({ 
                        ...editingHomework, 
