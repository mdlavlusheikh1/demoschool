...
function AttendanceReportPage() {
  ...
  useEffect(() => {
    if (!authLoading && userData) {
      if (userData.role !== 'teacher' && userData.role !== 'admin' && userData.role !== 'super_admin') {
  ...
  const loadReportData = async () => {
    ...
    try {
      ...
      // Get schoolId from userData or settings
  ...
  const loadTeacherReportData = async () => {
    ...
    try {
      ...
      } else if (selectedReportType === 'summary' || selectedReportType === 'class') {
        // Summary/Class report - get attendance for date range
        if (!startDate || !endDate) {
          alert('অনুগ্রহ করে শুরুর তারিখ এবং শেষের তারিখ নির্বাচন করুন');
          setLoading(false);
          return;
      ...
      alert('শিক্ষক রিপোর্ট লোড করতে সমস্যা হয়েছে');
    } finally {
      setLoading(false);
    }
  };
