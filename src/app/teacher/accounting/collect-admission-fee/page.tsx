...
import { sendFeePaymentNotification } from '@/lib/fee-notification-helper';
...
function CollectAdmissionFeePage() {
  ...
  // Refs to track transaction count and total for real-time updates
  const lastTransactionCountRef = useRef<number>(0);
  const previousTotalPaidRef = useRef<number>(0);
  const isInitialLoadRef = useRef<boolean>(true);
  
  // Helper function to format roll number
  const formatRollNumber = (rollNumber: string | undefined, studentId: string) => {
    const rollNumRegex = new RegExp('^STD0*', 'i');
    return (rollNumber || studentId || '').replace(rollNumRegex, '');
  };
  
  // Helper function to extract numeric part from class name
  const extractClassNumber = (className: string | undefined) => {
    const nonDigitRegex = new RegExp('\\D', 'g');
    return parseInt(className?.replace(nonDigitRegex, '') || '1');
  };
  
  // Helper function to extract year from voucher number
  const extractVoucherYear = (voucherNumber: string | undefined) => {
    const yearMatch = voucherNumber?.match(new RegExp('ADM-(\\d{4})'));
    return yearMatch?.[1];
  };
  
  // Helper function to match voucher number pattern
  const matchVoucherPattern = (voucher: string, year: string) => {
    const pattern = new RegExp(`ADM-${year}-(\\d+)`);
    return voucher.match(pattern);
  };
  ...
  useEffect(() => {
    ...
    const unsubscribe = onAuthStateChanged(auth, (firebaseUser) => {
      if (firebaseUser) {
        ...
        // Set up real-time listener for admission fee transactions
        const schoolId = SCHOOL_ID;
        
        // Use accountingQueries.subscribeToTransactions to listen to all transactions
        // and filter for admission_fee and session_fee in the callback
        transactionUnsubscribe = accountingQueries.subscribeToTransactions(
          schoolId,
          (transactions) => {
            // Filter for admission fee related transactions
            const admissionFeeTransactions = transactions.filter(t => 
              (t.category === 'admission_fee' || t.category === 'session_fee') && t.status === 'completed'
            );
            
            // Get the latest transaction timestamp to detect new transactions
            const latestTransaction = admissionFeeTransactions.length > 0
              ? admissionFeeTransactions.sort((a, b) => {
                  const dateA = new Date(a.collectionDate || a.date || a.createdAt || 0).getTime();
                  const dateB = new Date(b.collectionDate || b.date || b.createdAt || 0).getTime();
                  return dateB - dateA;
                })[0]
              : null;
            
            const latestTransactionTime = latestTransaction 
              ? new Date(latestTransaction.collectionDate || latestTransaction.date || latestTransaction.createdAt || 0).getTime()
              : 0;
            
            // Calculate total paid amount from all transactions to detect changes
            const totalPaidFromTransactions = admissionFeeTransactions.reduce((sum, t) => {
              return sum + (t.paidAmount || t.amount || 0);
            }, 0);
            
            // Reload if:
            // 1. Not initial load AND
            // 2. (Transaction count changed OR total paid amount changed OR we have a very recent transaction - within last 30 seconds)
            const isRecentTransaction = latestTransactionTime > (Date.now() - 30000); // 30 seconds ago
            const totalChanged = totalPaidFromTransactions !== previousTotalPaidRef.current;
            
            if (!isInitialLoadRef.current && 
                (admissionFeeTransactions.length !== lastTransactionCountRef.current || 
                 totalChanged || 
                 isRecentTransaction)) {
              console.log('🔄 Admission fee transaction updated, reloading data...', {
                previous: lastTransactionCountRef.current,
                current: admissionFeeTransactions.length,
                previousTotal: previousTotalPaidRef.current,
                currentTotal: totalPaidFromTransactions,
                latestTransactionTime: latestTransactionTime ? new Date(latestTransactionTime).toISOString() : 'none',
                isRecent: isRecentTransaction,
                totalChanged: totalChanged
              });
              loadData();
            }
            
            lastTransactionCountRef.current = admissionFeeTransactions.length;
            previousTotalPaidRef.current = totalPaidFromTransactions;
    ...
    try {
      ...
      const processedStudents: AdmissionStudent[] = studentsData
        ...
        .map((student, index) => {
          ...
          // Check both uid and studentId to match transactions (same logic as admin panel)
          ...
          } else if (extractClassNumber(student.class) > 5) {
          ...
          // Use paidAmount if available, otherwise use amount (same logic as admin panel)
          ...
          // Calculate status based on due amount (same logic as admin panel)
          ...
          return {
            studentId: finalStudentId, // Use the already calculated finalStudentId (same as admin panel)
  ...
  const openPaymentDialog = async (student: AdmissionStudent) => {
    ...
    try {
      ...
      const admissionTransactions = allTransactions.filter((t: any) => {
        ...
        const voucherYear = extractVoucherYear(t.voucherNumber);
      ...
      const sequences = voucherNumbers
        .map((v: string) => {
          const match = matchVoucherPattern(v, currentYear.toString());
      ...
      let collectorName = 'User';
      if (userData?.name) {
        collectorName = userData.name;
      } else if (authUser?.displayName) {
        collectorName = authUser.displayName;
      } else if (authUser?.email) {
        collectorName = authUser.email.split('@')[0];
      }
      ...
      const collectorName = userData?.name || authUser?.displayName || authUser?.email?.split('@')[0] || 'Admin User';
  ...
  const handleSubmitPayment = async () => {
    ...
    try {
      ...
      // Send fee payment notification (SMS, push, email) - respects notification settings
      try {
        const feeType = selectedStudent.studentType === 'new_admission' ? 'ভর্তি ফি' : 'সেশন ফি';
      ...
      setAdmissionStudents(prevStudents =>
        prevStudents.map(student => {
          if (student.studentId === selectedStudent.studentId) {
            ...
            const newTotalDiscount = student.totalDiscount + discount;
  ...
  return (
    <TeacherLayout title="ভর্তি ও সেশন ফি আদায়" subtitle="শিক্ষার্থীদের ভর্তি এবং সেশন ফি সংগ্রহ করুন">
      <div className={`p-6 lg:p-8 min-h-screen transition-all duration-300 ${dialogOpen ? 'bg-gradient-to-br from-blue-900 via-gray-900 to-purple-900 bg-opacity-60 backdrop-blur-sm' : 'bg-gray-50'}`}>
        ...
        {dialogOpen && selectedStudent && (
          <div className="fixed inset-0 bg-black bg-opacity-60 backdrop-blur-sm flex items-center justify-center z-[100] p-4 animate-in fade-in duration-300">
            <div className="bg-white rounded-2xl shadow-2xl max-w-2xl w-full overflow-y-auto transform transition-all duration-300 ease-out scale-100 opacity-100 animate-in zoom-in-95 duration-300" style={{ maxHeight: '90vh' }}>
              {/* Dialog Header */}
              <div className="flex items-center justify-between p-6 border-b border-gray-200">
                <div>
                  ...
                  <p className="text-sm text-gray-600">
                    রোল: {formatRollNumber(selectedStudent.rollNumber, selectedStudent.studentId)} | শ্রেণি: {selectedStudent.className} |
        ...
        {notification.show && (
          <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-[100]">
            <div className={`bg-white rounded-lg p-6 max-w-sm w-full mx-4 shadow-xl ${notification.type === 'success' ? 'border-l-[4px] border-green-500' : 'border-l-[4px] border-red-500'}`}>
              <div className="flex items-center">
                <div className={`flex-shrink-0 w-8 h-8 rounded-full flex items-center justify-center mr-3 ${notification.type === 'success' ? 'bg-green-100' : 'bg-red-100'}`}>
                ...
                <button
                  ...
                  className={`ml-3 flex-shrink-0 ${notification.type === 'success' ? 'text-green-400 hover:text-green-600' : 'text-red-400 hover:text-red-600'}`}
                >
                  <X className="w-5 h-5" />
                </button>
              </div>
