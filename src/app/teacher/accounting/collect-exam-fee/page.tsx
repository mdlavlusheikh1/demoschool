...
function CollectExamFeePage() {
    ...
    try {
      const transactionPromises = selectedStudents.map(student => {
        ...
        const transactionData: Omit<FinancialTransaction, 'id' | 'createdAt' | 'updatedAt'> = {
          ...
          collectedBy: userData?.name || user?.displayName || user?.email?.split('@')[0] || 'Admin User',
      ...
      const feeCollectionPromises = selectedStudents.map((student, index) => {
        ...
        return feeQueries.createFeeCollection({
          ...
          collectedBy: user?.displayName || userData?.name || user?.email?.split('@')[0] || 'Admin User',
          schoolId: SCHOOL_ID
        });
      });
  ...
  return (
    <TeacherLayout title="পরীক্ষার ফি আদায়" subtitle="শিক্ষার্থীদের পরীক্ষার ফি সংগ্রহ করুন">
